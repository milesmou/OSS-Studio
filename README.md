...
- 支持按钮选择或拖拽上传文件和目录，并将单个 Bucket 的并发上传限制为 3 路。
- 支持批量下载、批量删除、创建 OSS 目录、同 Bucket 复制和取消当前传输任务。
...
应用默认以空工作区启动，Bucket 配置保存在本地，AccessKey 保存至 Windows 凭据管理器。对象浏览、上传、下载、删除、创建目录和同 Bucket 复制均已接入真实 OSS；完整的跨 Bucket 复制和持久化传输队列仍待继续完善。
